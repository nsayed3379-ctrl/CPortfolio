...
import { FAQ_LIST_QUERY, PRODUCT_LIST_QUERY, CASE_STUDY_LIST_QUERY } from "@/sanity/queries";
import { selectFeatured } from "@/lib/selectFeatured";
import type { FaqDoc, ProductDoc, CaseStudyDoc } from "@/sanity/types";
...
export default async function Home() {
  ...
  const [settings, faqs, products, work] = await Promise.all([
    getSiteSettings(),
  ...
  // curated highlight reel, not a full browse of every Product/Work
  ...
  return (
    <>
      ...
      {(FEATURES.products || FEATURES.work) && (
        <ExploreShowcase products={featuredProducts} work={featuredWork} />
      )}
